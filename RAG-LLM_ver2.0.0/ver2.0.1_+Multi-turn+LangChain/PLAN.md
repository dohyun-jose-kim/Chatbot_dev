...
실행은 `RAG-LLM_ver2.0.0/` 한 곳에서만 한다. 런타임 흐름이 폴더 번호대로 흐르도록 `NN_` 다단계로 구성한다.

```
RAG-LLM_ver2.0.0/
├── config.py                    # 공유 설정: ver1 chroma_db 절대경로, 모델명, TOP_K, 프롬프트
├── requirements.txt
├── 00_Docs/                     # 문서 (설계 메모, 사용법 등)
├── 01_retrieval/
│   └── pubmedbert_retriever.py  # ver1 검색 로직 → LangChain BaseRetriever 어댑터
├── 02_chain/
│   └── rag_chain.py             # 멀티턴 체인: ChatOllama + history-aware retriever
├── 03_memory/
│   └── db_utils.py              # 세션별 대화기록 SQLite (레퍼런스 차용)
├── 04_interface/
│   └── chat.py                  # CLI 멀티턴 루프 (진입점, ver1 chatbot.py 차용)
└── ver2.0.1_+Multi-turn+LangChain/
    └── PLAN.md                  # 이 문서
```

**import 규칙**: `NN_` 폴더는 숫자로 시작해 Python `import` 불가. 진입점(`chat.py`)에서만
각 스테이지 폴더를 `sys.path`에 등록하고, 중간 모듈은 파일명으로 서로 import한다.
개발 중 단일 스테이지 테스트는 `PYTHONPATH=.:01_retrieval:02_chain:03_memory` 로 실행.
ver1 디렉터리는 import하지 않고 필요한 코드를 복사해온다 (ver1 동결 원칙).

**ver1 chroma_db 부작용 주의**: chromadb는 쿼리 시 sqlite를 read-write로 열어 파일이
변경됨으로 표시될 수 있다. ver1 동결 유지를 위해 그 변경은 절대 커밋하지 않는다
(`git checkout -- .../chroma.sqlite3`). 런타임에는 무해.
